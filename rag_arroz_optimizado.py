"""
Sistema RAG Optimizado Específicamente para Arroz
VERSIÓN COMPATIBLE CON CHROMADB 1536D - USA OPENAI EMBEDDINGS
...
@dataclass
class PesticideInfo:
    """Estructura completa para información de pesticidas para arroz"""
    name: str
    ...
    crops: List[str] = None
    crop_stage: str = ""  # Preemergente, postemergente, macollaje, primordio, etc.
...
class RiceRAGSystem:
    """Sistema RAG optimizado para pesticidas de arroz"""

    def __init__(self, persist_directory: str = "./chroma_arroz_v1"):
        ...
        # Crear colección específica para arroz
        self.collection = self.client.get_or_create_collection(
            name="pesticidas_arroz",
            metadata={"description": "Pesticidas específicos para cultivo de arroz"}
        )

        logger.info(f"[RAG Arroz] Inicializado con {self.collection.count()} documentos")
    ...
    def extract_pesticide_info(self, text: str, source_file: str) -> List[PesticideInfo]:
        ...
        active_ingredient_patterns = [
            ...
            r'(propanil|bispiribac|oxadiazon|penoxsulam|clomazone|acetoclor|cyhalofop|quinclorac|butaclor|molinate|propiconazole|azoxystrobin|tricyclazole|difenoconazole|isoprothiolane|lambda-cyhalothrin|fenitrothion|etofenprox|thiamethoxam|chlorantraniliprole)\s+(\d+(?:[,\.]\d+)?)\s*(?:%|g/L|g/kg)'
        ]

        dose_patterns = [
            r'(\d+(?:\.\d+)?)\s*(?:-\s*(\d+(?:\.\d+)?))?\s*(l/ha|g/ha|litros/ha|gramos/ha|ml/ha|kg/ha|cc/ha|lt/ha)',
            r'(\d+(?:\.\d+)?)\s*(?:-\s*(\d+(?:\.\d+)?))?\s*(litros?\s*por\s*hect[aá]rea|gramos?\s*por\s*hect[aá]rea)',
        ...
        pest_patterns = [
            ...
            r'(?:hualcacho|echinochloa|arroz rojo|oryza|ciper[aá]ceas|scirpus|cuscuta|dodder|paspalum|piren|piricularia|tiz[oó]n de la vaina|mancha parda|chinche del grano|sogata|mosquito del arroz|carb[oó]n|roya)',
        ]

        timing_patterns = [
            r'(?:aplicar|aplicación)\s+([^\n\.]+?)(?:antes|después|durante)',
            r'(?:preemergente|pre-emergente|postemergente|post-emergente)',
            r'(?:macollaje|ahijamiento|primordio floral|primordio|espigamiento|floración|llenado de grano|maduración|trasplante)',
        ...
        for pesticide in pesticides:
            ...
            for pattern in dose_patterns:
                ...
                if matches and not pesticide.dose_range:
                    ...
                    if isinstance(sample, tuple):
                        if len(sample) >= 3 and sample[1]:
                            pesticide.dose_range = f"{sample[0]}-{sample[1]}"
                            pesticide.dose_unit = sample[2]
                        elif len(sample) >= 2:
                            pesticide.dose_range = sample[0]
                            pesticide.dose_unit = sample[1] if len(sample) > 1 else ""
                        else:
                            pesticide.dose_range = sample[0]
                    else:
            ...
            if 'arroz' in text_lower:
                pesticide.crops.append('arroz')
            ...
            elif 'trasplante' in text_lower:
                pesticide.crop_stage = 'Trasplante'
            elif 'macollaje' in text_lower or 'ahijamiento' in text_lower:
                pesticide.crop_stage = 'Macollaje'
            elif 'primordio' in text_lower:
                pesticide.crop_stage = 'Primordio'
            ...
            elif 'llenado' in text_lower and 'grano' in text_lower:
                pesticide.crop_stage = 'Llenado de grano'
    ...
    def _improve_dose_by_crop_context(self, pesticides: List[PesticideInfo], text: str) -> List[PesticideInfo]:
        ...
        for i, line in enumerate(lines):
            ...
            if 'arroz' in line_lower and len(line_lower) < 60:
                        ...
                        if matches:
                            for pesticide in pesticides:
                                ...
                                if isinstance(match, tuple) and len(match) == 2:
                                    pesticide.dose_range = f"{match[0]} - {match[1]}"
                                else:
                                    pesticide.dose_range = str(match[0])

                                context = ' '.join(lines[max(0, i - 2):min(len(lines), i + 5)])
                                context_lower = context.lower()
                                if 'g/100' in context_lower:
                                    pesticide.dose_unit = 'g/100 L de agua'
                                elif 'kg/ha' in context_lower:
                                    pesticide.dose_unit = 'kg/ha'
                                elif 'l/ha' in context_lower or 'lt/ha' in context_lower:
    ...
    def _is_valid_pesticide_name(self, name: str) -> bool:
        ...
        invalid_words = [
            ...
            'antes', 'usar', 'lote', 'fabricante', 'arroz', 'cereal'
        ]
    ...
    def _process_table_by_rows(self, table: List, page_num: int, table_num: int) -> List[Dict[str, Any]]:
        ...
        cultivo_col = self._find_column_index(headers, ['cultivo', 'crop', 'arroz'])
    ...
    def _create_structured_row_chunk(self, headers: List[str], row: List, row_idx: int,
        ...
        if dosis:
            ...
            if 'g/100' in dosis_header.lower():
                chunk_text += f"UNIDAD: g/100 L de agua\n"
            elif 'kg/ha' in dosis_header.lower() or 'kg/há' in dosis_header.lower():
                chunk_text += f"UNIDAD: kg/ha\n"
            elif 'l/ha' in dosis_header.lower() or 'l/há' in dosis_header.lower():
                chunk_text += f"UNIDAD: L/ha\n"
            elif 'g/ha' in dosis_header.lower() or 'g/há' in dosis_header.lower():
    ...
    def process_pdf(self, pdf_path: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"[RAG Arroz] Procesando: {filename}")
        ...
        logger.info(f"[RAG Arroz] ✅ {filename}: {len(chunks)} chunks creados ({len(table_chunks)} de tablas)")
        return chunks
    ...
    def add_documents(self, chunks: List[Dict[str, Any]]):
        ...
        logger.info(f"[RAG Arroz] Generando {len(texts)} embeddings con OpenAI API...")
        embeddings = []
        for i, text in enumerate(texts):
            if i % 10 == 0:
                logger.info(f"[RAG Arroz] Embeddings: {i}/{len(texts)}")
            embedding = self.get_embedding(text)
            embeddings.append(embedding)

        logger.info(f"[RAG Arroz] ✅ {len(embeddings)} embeddings generados (1536D)")
        ...
        logger.info(f"[RAG Arroz] ✅ {len(chunks)} chunks añadidos a la base")

    def search(self, query: str, top_k: int = 10) -> List[Dict[str, Any]]:
        logger.info(f"[RAG Arroz] 🔍 Buscando: '{query[:50]}...'")

        query_embedding = self.get_embedding(query)

        logger.info(f"[RAG Arroz] Embedding query: {len(query_embedding)}D")
        ...
        if results["documents"] and results["documents"][0]:
            ...
            logger.info(f"[RAG Arroz] ✅ Encontrados {len(formatted_results)} resultados")
        else:
            logger.warning(f"[RAG Arroz] ⚠️  No se encontraron resultados")
...
class RiceRAGSystemEnhanced(RiceRAGSystem):
    ...
    def _extract_relevant_crops(self, cultivos_raw: str) -> str:
        ...
        crop_patterns = [
            r'\b(arroz|cereales|maíz|sorgo|trigo)\b'
        ]
...
def process_documents_directory(rag_system: RiceRAGSystem, documents_dir: str = "./documents_arroz"):
    ...
    logger.info(f"[RAG Arroz] Encontrados {len(pdf_files)} archivos PDF")
    ...
    for pdf_file in pdf_files:
        try:
            logger.info(f"[RAG Arroz] Procesando: {pdf_file.name}")
            chunks = rag_system.process_pdf(str(pdf_file))
            if chunks:
                rag_system.add_documents(chunks)
                processed_count += 1
                logger.info(f"[RAG Arroz] ✅ {pdf_file.name} procesado exitosamente")
            else:
                logger.warning(f"[RAG Arroz] ⚠️ {pdf_file.name} no generó chunks")
        except Exception as e:
            error_count += 1
            logger.error(f"[RAG Arroz] ❌ Error procesando {pdf_file.name}: {e}")
            continue

    total_docs = rag_system.count()
    logger.info(f"[RAG Arroz] 🎉 Procesamiento completo:")
...
if __name__ == "__main__":
    print("=" * 60)
    print("🍚 SISTEMA RAG ARROZ - VERSIÓN COMPATIBLE CHROMADB 1536D")
    print("=" * 60)
    ...
    try:
        ...
        print(f"   - Modelo embeddings: text-embedding-3-small (OpenAI)")
        print(f"   - Dimensión embeddings: 1536")
        ...
        print(f"\n🔍 Prueba de búsqueda:")
        test_queries = [
            "herbicida arroz postemergente",
            "fungicida piricularia arroz",
            "dosis propanil arroz"
        ]
